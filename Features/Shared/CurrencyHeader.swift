    ...
    private var statsRow: some View {
        HStack(spacing: isCompactHeight ? 6 : 10) {
            ...
            if game.autoJumpsPerSecond > 0 {
                StatTile(label: "Auto", value: String(format: "%.1f/s", game.autoJumpsPerSecond), tint: Theme.Colors.accent)
            }
            if game.prestigeMultiplier > 1 || game.state.achievementMultiplier > 1 {
                StatTile(label: "Global", value: game.globalMultiplierText, tint: Theme.Colors.prestige)
            }
        }
    }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: Theme.Spacing.sm) {
            ...
            ProgressView(value: game.canPrestige ? 1 : game.prestigeProgressFraction)
                .tint(Theme.Colors.prestige)
        }
        .padding(Theme.Spacing.md)
        .background(Theme.Colors.surface, in: RoundedRectangle(cornerRadius: Theme.Metrics.cardRadius))
